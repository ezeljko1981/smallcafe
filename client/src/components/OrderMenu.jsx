...
function OrderMenu() {
 ...
 const onSetTableId = useCallback((tableId: Int) =>{
  ...
  if (getArticles.error)    return <React.Fragment>Error :(.</React.Fragment>;
    ...
    <div className="App">
      ...
      <form onSubmit={e => {
          ...
          createGuestOrder({ variables: { id: orderIdRandom, id_table: arrayOfOrders[0], price: totalPrice }})
              .catch(error => {
              setErrorReport("Error inserting your order! ");
            });
            ...
            .map(article => (
              ...
              }}).catch(error => { setErrorReport("Error inserting your order! "); })
            ));
            setErrorReport("");
      ...
      {currentTable > 0 &&
        ...
        <p><b> MENU:</b></p>
          {getArticles.data.getArticles.map(article => (
              <div key={article.id}>
               <p>
